...
class ConfigurationLoader {
    ...
    static async getTsConfig(tsConfigPath) {
        const tsConfigFile = await fs_extra_1.readFile(tsConfigPath);
        const tsConfig = JSON.parse(strip_json_comments_1.default(tsConfigFile.toString()));
        if (!Object.keys(tsConfig).includes('extends')) {
            return tsConfig;
        }
        const tsConfigFolderPath = path_1.dirname(tsConfigPath);
        const tsConfigExtendsPath = path_1.isAbsolute(tsConfig.extends) ? tsConfig.extends : path_1.join(tsConfigFolderPath, tsConfig.extends);
        const baseTsConfig = await ConfigurationLoader.getTsConfig(tsConfigExtendsPath);
        return Utils_1.Utils.merge(baseTsConfig, tsConfig);
    }
